...
[点击下载项目演示视频](https://github.com/JunJie010/four_axis-robot_arm/blob/main/%E5%88%B6%E4%BD%9C%E8%BF%87%E7%A8%8B%E7%BA%AA%E5%BF%B5/%E6%88%90%E5%93%81%E5%B1%95%E7%A4%BA.mp4)

## 项目功能

- **手机蓝牙遥控**  
  基于 BLE 低功耗蓝牙协议，可通过手机 APP 连接机械臂，实现四轴关节的独立控制和夹爪开合，控制距离满足桌面级应用需求。

- **多舵机协同运动**  
  机身采用 3 个 MG996R 舵机驱动大臂、小臂和底座旋转，夹爪采用 SG90 舵机，通过自定义定时器调度算法实现多舵机同步运动，并可调节转动速度。

- **精确角度控制**  
  基于正逆运动学算法，将机械臂末端位置映射为各关节舵机角度，支持点到点运动控制，可完成指定位置的抓取与释放任务。

- **夹爪抓取功能**  
  夹爪采用双齿轮对称结构设计，最大抓取直径约 3cm，可轻松夹取积木、螺丝等小型物件，夹取力度适中，成功率超过 83%。

- **结构自研设计**  
  从零进行机械结构设计，包含固定底座、旋转底座、平行四边形连杆机构和夹爪模块，全部零部件通过 3D 打印制作并手工组装。

- **主控板自研**  
  自主绘制 ESP32 主控板，集成供电电路、舵机驱动接口、蓝牙模块和调试串口，实现硬件与软件的完全自主可控。
